...
| GP-8 | Imports `ogcincubator.github.io/bblocks-eoap-cct` (the fork) rather than the GeoLabs origin; a register importing both (this one) sees `eoap.cct.*` twice | register config | this register imports the GeoLabs origin (Q-IMPORT) | import the origin |
