...
| Q-W3-RUN | **Resolved 2026-09-23.** All 6 W3 profiles are built from run record `water-bodies`: the pinned `mastering-app-package` source (40ecc09, `app-water-bodies-cloud-native.cwl#water-bodies`) run with `cwltool --enable-ext --provenance` against a locally patched copy (`docs/DEVIATIONS.md` U-06), inputs from the source's own `water-bodies/params.yml`, ~9 min, two STAC items scattered over 2 bands each. Engine is cwltool, same caveat as Q-W2-RUN (not a DRU/Calrissian run). |
