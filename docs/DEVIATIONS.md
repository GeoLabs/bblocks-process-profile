...
| M-13 | A packed `$graph` document with **more than one** top-level `Workflow`-class element is always converted from the *first* one (`map(select(.class == "Workflow")) \| first`); an explicitly requested non-first Workflow (by CWL `id`) is silently ignored | `water-bodies.detect-water-body` (the `#water-bodies` and `#detect_water_body` sub-workflow both have `class: Workflow` in the same packed document; requesting `#detect_water_body` still returned `#water-bodies`'s own inputs/outputs) | **Documented + generator step**: `scripts/generate.py` `load_process()` reorders the document's `$graph` so the requested element is first before handing it to the transform, leaving every other member (including the ones the requested Workflow steps into) untouched. Neither W1 nor W2 exercises this, each packs exactly one top-level Workflow |
...
`gfenoy/mastering-app-package`'s `main` branch is itself a live OGC API - Processes deployment
reference the author actively uses outside this register; nothing here is applied to that clone or
pushed upstream — `scripts/run_workflows.py`'s `workflow.patch` (`scripts/sources.yaml`) rewrites a
*copy* of the CWL file in a temporary directory, immediately before handing it to `cwltool`.

| ID | Issue | Handling |
|---|---|---|
| U-06 | `app-water-bodies-cloud-native.cwl`'s `water-bodies` Workflow declares `requirements: [{class: NetworkAccess, networkAccess: true}]` with no namespace prefix. Bare `NetworkAccess` is not a CWL v1.0 requirement class — it is cwltool's own extension (`cwltool:NetworkAccess`, `http://commonwl.org/cwltool#`) — so `cwltool --validate` fails with `Field 'class' contains undefined reference`, before any run starts | Local-only patch (never touching the clone): add `$namespaces: {cwltool: http://commonwl.org/cwltool#}` and rewrite the requirement to `class: cwltool:NetworkAccess`, then invoke `cwltool --enable-ext` (both the prefix *and* the flag are required; either alone still fails validation). If this is ever fixed upstream the patch's `find` anchor stops matching and `run_workflows.py` fails loudly rather than silently no-op |
